...
BOOL CClaseTransformacionesDlg::OnInitDialog()
{
    ...
    Cx=320; 
    Cy=240; 
/*numero de punto que tiene la curva   ****-1***  menos uno*/
	n=7;
	densidad = 100;
...
{   
	...
	//Proceso de An,
	//Obtener_An();  NO ES NECESARIO , ERA PARA NEWTON
	...
	for (int l=1; l<=n; l++)
	{
		...
		for (int i=1; i<=n; i++)
		{
			double w1;
			w1 = W(i,tt);
			PtoAux[1] = (PtoAux[1] + (p3d[i][1] * W(i,tt)));
			PtoAux[2] = (PtoAux[2] + (p3d[i][2] * W(i,tt)));
			PtoAux[3] = (PtoAux[3] + (p3d[i][3] * W(i,tt)));
		}
...
/*
CURVA DE LAGRANGE
alfa(t) = sumatoria de Pi*Li(t) , t E [t0,tn]
...
/*esta funcion devuelve el polinomio de Lagrange*/
double CClaseTransformacionesDlg::W(int k, double tt)
{
	double L;
	L=1;
